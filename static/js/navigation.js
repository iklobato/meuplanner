/**
 * Navigation functionality for Meu Controle Financeiro
 */
...
function setupNavigation() {
  ...
  document.addEventListener('click', function(event) {
    ...
    if (!isClickInsideSidebar && !isClickOnMenuToggle && 
        window.innerWidth <= 576 && sidebar.classList.contains('active')) {
  ...
  bodyChildren.forEach(child => {
    ...
    if (
      ...
      (child.tagName === 'SCRIPT' && 
       (child.src.includes('navigation.js') || child.hasAttribute('data-mpf-nav')))
    ) {
      return;
    }
